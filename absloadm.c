...
int R1,                                           /*номер 1-го регистра-опе-*/
    ...
    L,                                             /*Длина опреандов */
    						  /*в формате SS            */
    B2,                                            /*номер базового регистра */
						  /*второго операнда в формате SS            */
    D2;                                            /*смещение в формате SS  */
						  /*второго операнда                   */
unsigned char C_PRIZNAK = 3;
...
int P_MVC()                                         /* п р о г р а м м а      */
/* реализации семантики   */
 {                                                /* команды S              */
  ADDR = VR[B] + D;                       /*вычисление абс.адреса и */
  int sm = ( int ) ( ADDR -I );                       /*смещения                */
  ADDR = VR[B2] + D2;                       /*вычисление абс.адреса и */
  int sm2 = ( int ) ( ADDR -I );                       /*смещения                */
  for (i = 0; i < L + 1; i++) 
  {
...
int P_CH()                                         /* п р о г р а м м а      */
/* реализации семантики   */
 {                                                /* команды S              */
  ADDR = VR[B] + VR[X] + D;                       /*вычисление абс.адреса и */
  int sm = ( int ) ( ADDR -I );                       /*смещения                */               /*второго операнда в сог- */
  unsigned long var1 = VR[R1] % 0x10000;
    unsigned long var2 = OBLZ[BAS_IND + CUR_IND + sm] * 0x100+/*формирование содержимого*/
  OBLZ[BAS_IND + CUR_IND + sm + 1]; /*лашениях ЕС ЭВМ         */
  if (var1 == var2) 
  {
      C_PRIZNAK = 0;
  }
