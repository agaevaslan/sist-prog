#define DL_ASSTEXT 25
#define DL_OBJTEXT 50                             /*длина об'ектн. текста   */
#define NSYM 15                                   /*размер табл.символов    */
...
//    int B2D2;                                     /*X2 - второй операнд     */
  ...
  union                                           /*определить об'единение  */
   ...
   {
    unsigned char OP;                             /*код операции            */
    unsigned char L;                              /*уменьшенная на 1 длина оп-да*/
    short B1D1;                                   /*B1 - баз.рег.1-го оп-да */
    short B2D2;                                   /*D1 - смещен.относит.базы*/                               
						                          /*B2 - баз.рег.2-го оп-да */
						                          /*D2 - смещен.относит.базы*/
   };
  ...
  union                                           /*определить об'единение  */
   ...
   unsigned char BUF_VAR_VALUE [8];               /*буфер для знач. переменной*/
   ...
   {                                              /*то:                     */
    if                                            /* если псевдооперация DC */
     ...
     )                                            /* следующее:             */
     {
      ...
      CHADR = CHADR + T_SYM[ITSYM].DLSYM;         /*увелич.CHADR на дл. символа*/
     }
     else if                                            /* если псевдооперация DC */
     (                                            /* определяет константу   */
      TEK_ISX_KARTA.STRUCT_BUFCARD.OPERAND[0]=='H'/* типа Р, то выполнить   */
     )                                            /* следующее:             */
     {
     ...
     (                                            /* определяет константу   */
      TEK_ISX_KARTA.STRUCT_BUFCARD.OPERAND[0]=='P'/* типа P, то выполнить   */
     )                                            /* следующее:             */
     {
       T_SYM[ITSYM].DLSYM = atoi(strtok           /* Определяем длину конст.*/                      
	     (                                        /* Берём число между PL и '*/
           (char*) TEK_ISX_KARTA.STRUCT_BUFCARD.OPERAND + 2, 
	        "'"                                   
	     ));
      T_SYM[ITSYM].PRPER = 'A';                   /*  а,призн.перемест.='A' */
      PRNMET = 'N';                               /*  занулить PRNMET зн.'N'*/
      CHADR = CHADR + T_SYM[ITSYM].DLSYM;         /*увелич.CHADR на дл. символа*/
     }
    else
...
int FDS()                                         /*подпр.обр.пс.опер.DS    */
{
  if ( PRNMET == 'Y' )                            /*если псевдооп.DS помеч.,*/
   {                                              /*то:                     */
    if                                            /* если псевдооперация DS */
     (                                            /* определяет константу   */
      TEK_ISX_KARTA.STRUCT_BUFCARD.OPERAND[0]=='F'/* типа F, то выполнить   */
     )                                            /* следующее:             */
     {
     ...
     else if                                      /* если псевдооперация DS */
     (                                            /* определяет константу   */
...
void STXT( int ARG , int IS_VAR)                  /*подпр.формир.TXT-карты  */
 {
  ...
  memset ( TXT.STR_TXT.OPER , 64 , 8 );           /*формирование поля OPER  */
  if (!IS_VAR) {                                  /*Если записываем опер.*/
    if ( ARG == 2 )                               /*Если дл. опер. равна 2   */
     {                                            /*То это RR команда        */
      memcpy ( TXT.STR_TXT.OPER,RR.BUF_OP_RR , 2 ); /* Копируем 2 байта      */
      TXT.STR_TXT.DLNOP [1] = 2;                   /* Выставляем дл. 2 байта */
     }
    else if ( ARG == 4 )                          /*Если дл. опер. равна 4   */
     {                                            /*То это RX команда        */
      memcpy ( TXT.STR_TXT.OPER , RX.BUF_OP_RX , 4); /* Копируем 4 байта      */
      TXT.STR_TXT.DLNOP [1] = 4;                    /* Выставляем дл. 4 байта */
     }
    else if ( ARG == 6 )                          /*Если дл. опер. равна 6   */
     {                                            /*То это SS команда        */
      memcpy ( TXT.STR_TXT.OPER , SS.BUF_OP_SS , 6); /* Копируем 6 байт      */
      TXT.STR_TXT.DLNOP [1] = 6;                    /* Выставляем дл. 6 байт */
     }
  } else {                                          /* Записываем константу */    
      memcpy ( TXT.STR_TXT.OPER , BUF_VAR_VALUE , ARG);/* Копируем знач из буфф. знач.*/
      TXT.STR_TXT.DLNOP [1] = ARG;                     /* Выставляем дл. ARG байт */
  }
...
int SDC()                                         /*подпр.обр.пс.опер.DC    */
 {
  char * METKA;                                   /* Метка для писка в т. сим.*/
  char * DC_METKA = strtok                        /* Вырезаем метку из карты*/         
	   (                                     
        (char*) TEK_ISX_KARTA.STRUCT_BUFCARD.METKA, 
	    " "                                   
	   );
  int DLSYM;                                      /* Поле с длин. конст.   */ 
  for ( int J=0; J<=ITSYM; J++ )                  /* Посик симв. в т. сим. */   
  {                                              
    METKA = strtok (                              /* Вырезаем метку из имени*/ 
        (char*) T_SYM[J].IMSYM , " "
       );
    if( !strcmp ( METKA , DC_METKA ) )            /* Если метки совпали    */ 
    {                                          
	 CHADR = T_SYM[J].ZNSYM;                      /* Устанваливем сч. адрс.*/ 
     DLSYM = T_SYM[J].DLSYM;                      /* Устанваливем длин. конст.*/ 
	 goto SDC1;
    }                                          
  }                                              /* Если коснт. не найдена */ 
  return(1);                                     /* Выходим с ошибкой     */ 
   
  SDC1:
  memset ( BUF_VAR_VALUE , 64 , 8 );              /* Сброс буф. знач.       */
  memset ( BUF_VAR_VALUE , 0 , DLSYM );           /* Обнулеям буф. знач.    */ 
  ...
  if
    ...
    )                                             /* то                     */
   {
    ...
    int VALUE = atoi ( RAB );                     /*переводим занч в int    */
    swab ( &VALUE , &VALUE , 2 );                 /* и в соглашениях ЕС ЭВМ */
    memcpy (                                      /* копируем знач. в       */
	  BUF_VAR_VALUE+2,                            /* буф. знач с 3 поз      */
	  &VALUE,                    
	  2                          
	 );  
   }
    ...
    )                                             /* то                     */
   {
    ...
    swab ( &VALUE , &VALUE , 2 );                 /* и в соглашениях ЕС ЭВМ */
    memcpy (                                      /* копируем знач. в       */
	  BUF_VAR_VALUE,                              /* буф. знач с 3 поз      */
	  &VALUE,                     
	  2                           
	 );  
   }
   ...
   {
    strtok                                        /*смещ в АССЕМБЛЕРНОЙ      */
	 (                                            /*КАРТЕ на первый смивол ' */
    (char*)TEK_ISX_KARTA.STRUCT_BUFCARD.OPERAND+2,
	  "'"                                         
	 );
    RAB=strtok                                    /*загоняем в RAB          */
	 (                                            /* значение между '     ' */
    NULL,                                         
	  "'"                                         
	 );                                           /* вычисляем число тетрад */
    int TETR_COUNT = strlen(RAB);                 /* в знач без знака       */
    for (int J1 = 0; J1 < DLSYM*2-1; J1++) {      /* идём по тетрадам в буф */
        int VAR_TETR_INDEX = J1 - DLSYM*2         /* определ. тетраду числа */
          +1 + TETR_COUNT;                        /* соответ. тетр. в буф.  */
        char TETR_VAL;                            /* знач тетр. в буф.      */
        if (VAR_TETR_INDEX < 0) {                 /* если ещё не дошли      */
            TETR_VAL = 0x00;                      /* до знач. тетр. то 0    */
        } else {                                  /*если дошли до знач. тетр*/
            char CONV_INT_VAL[1] = {'0'};         /* буфер. для конвертации */
            CONV_INT_VAL[0] = RAB[VAR_TETR_INDEX];/*клаёдм ф буф. тетр. числа */
            int INT_VAL = atoi(CONV_INT_VAL);     /* конвертируем      */
            TETR_VAL = INT_VAL;                   /* кладём в терт. буф.    */
        }
        if (J1 % 2) {                             /* если ном. тетр. не чётный*/
            BUF_VAR_VALUE[J1/2] += TETR_VAL;      /* то кладём как есть     */
        } else {                                  /* если ном. тетр. чётный */
            BUF_VAR_VALUE[J1/2] += TETR_VAL * 16; /* то кладём со сдвигом   */
        }
    }
    BUF_VAR_VALUE[DLSYM - 1] += 12;               /* указываем знак числа + */
   }
  else                                            /*иначе                   */
   return (1);                                    /*сообщение об ошибке     */
  STXT (DLSYM, 1);                                /*формирование TXT-карты  */
  return (0);                                     /*успешн.завершение подпр.*/
 }
/*..........................................................................*/
int SDS()                                         /*подпр.обр.пс.опер.DC    */
 {
  char * METKA;                                   /* Метка для писка в т. сим.*/
  char * DS_METKA = strtok                        /* Вырезаем метку из карты*/ 
	   (                                     
    (char*) TEK_ISX_KARTA.STRUCT_BUFCARD.METKA,    
	    " "                                   
	   );
  int DLSYM;                                      /* Поле с длин. конст.   */ 
                                                  /* Посик симв. в т. сим. */  
  for ( int J=0; J<=ITSYM; J++ )                  
  {                                               /* Вырезаем метку из имени*/ 
            METKA = strtok (
		      (char*) T_SYM[J].IMSYM , " "
		     );                                   /* Если метки совпали    */ 
    if( !strcmp ( METKA , DS_METKA ) )            
    {                                             /* Устанваливем сч. адрс.*/ 
	 CHADR = T_SYM[J].ZNSYM;                      /* Устанваливем длин. конст.*/
     DLSYM = T_SYM[J].DLSYM;
	 goto SDS1;
    }                                             /* Если коснт. не найдена */ 
  }                                               /* Выходим               */ 
  ...
  SDS1:
  memset ( BUF_VAR_VALUE , 0x40 , 8 );            /* Сброс буф. знач.       */
  char *RAB;                                      /*рабочая переменная      */
  if
    (                                             /* если операнд не начинается*/
     !(
         (!memcmp(TEK_ISX_KARTA.STRUCT_BUFCARD.OPERAND,
	      "H", 1)                                 /* с комбинации  H         */
         ) || 
         (!memcmp(TEK_ISX_KARTA.STRUCT_BUFCARD.OPERAND,
	      "F", 1)                                 /* или с комбинации  F      */
         )
     )
    )                                             /* то                     */                                             
   return (1);                                    /*сообщение об ошибке     */
...
int SSS()                                         /*подпр.обр.опер.SS-форм. */
 {
  ...
  METKA1 = strtok                                /*в перем. c указат.METKA1*/ 
    ...
    (char*) METKA1,                              /*из певрого  опреанда    */
	    "+"                                      /*исх.текста АССЕМБЛЕРА   */
	   );
  SMESH = atoi(strtok                            /*в перем. c указат. SMESH*/      
	   (                                         /*выбираем знач. смещения */
    NULL,                                        /*из певрого  опреанда    */
	    "("                                      /*исх.текста АССЕМБЛЕРА   */
	   ));
  ARGLEN = strtok                                /*в перем. c указат. ARGLEN*/ 
	   (                                         /*выбираем знач. размера */
    NULL,                                        /*из певрого  опреанда    */
	    ")"                                      /*исх.текста АССЕМБЛЕРА   */
	   );
  SS.OP_SS.L = atoi(ARGLEN)-1;                   /*переносим знач. длины - 1*/
  if ( isalpha ( (int) *METKA1 )  || *METKA1 == '@')/*если лексема начинается */
   {                                              /*с буквы или @, то:        */
    for ( J=0; J<=ITSYM; J++ )                    /* все метки исх.текста в */
     {                                            /* табл. T_SYM сравниваем */
